...
if (parentMenu && filterButtons.length > 0) {
    ...
    filterButtons.forEach(function(filterButton) {
        filterButton.addEventListener('click', function(e) {
            ...
            const menuButton = allfoodItems.filter(function(food_item) {
                return food_item.category === category;
            });
...
const cartItem = document.querySelectorAll('.navigation__order-item-container');
...
function renderSubtotal(){
    ...
    subtotal.forEach((item) =>{
        item.innerHTML = `$${totalPrice.toFixed(2)}`;
    })
